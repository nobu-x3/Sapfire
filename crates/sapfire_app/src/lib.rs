pub mod components;
pub mod systems;
use components::{TransformComponent, CameraComponent};
use legion::*;
...
impl App{
    pub async fn new() -> (App, SapfireRenderer, winit::event_loop::EventLoop<()>){
        ...
        let systems = Schedule::builder().add_system(systems::input_system()).build();
        ecs.push((CameraComponent { camera: camera::Camera::new_perspective((45.0 as f32).to_radians(), 800.0, 600.0, 0.1, 100.0)} , TransformComponent::new()));
        resources.insert(CameraSpeed {speed: 3.0});
            ...
            event_loop.run(move |event, _, control_flow| match event {
                ...
                Event::RedrawRequested(window_id) if window_id == context.window().id() => {
                    ...
                    if let Some((camera_transform, camera)) = query.iter_mut(&mut app.ecs).next(){
                        ...
                        context.begin_scene(camera_transform.transform(), &camera_transform.translation, &camera.camera.projection);
                        match context.render() {
                            Ok(_) => {}
                            Err(sapfire_renderer::wgpu_wrapper::SurfaceError::Lost) => context.resize(context.size),
                            Err(sapfire_renderer::wgpu_wrapper::SurfaceError::OutOfMemory) => *control_flow = ControlFlow::Exit,
                            Err(e) => eprintln!("{:?}", e),
                        }
                    }
