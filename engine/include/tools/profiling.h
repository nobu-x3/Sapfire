	...
	public:
		...
		void BeginSession(const stl::string& name, const std::string& filepath = "results.json") {
			...
			if (m_CurrentSession) {
				...
				// instead.  That's better than having badly formatted profiling
				// output.
				if (Log::get_core_logger()) // Edge case: BeginSession() might
											// be before Log::Init()
				{
					CORE_ERROR("Instrumentor::BeginSession('{0}') when "
