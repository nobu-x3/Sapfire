...
{
      public:
      MovementComponent(class Actor *owner, int updateOrder = 10, bool newtonian = false)
	  : Component(owner, updateOrder), mAngularSpeed(0), mNetForce(Vector3::Zero), mMass(0),
	    mVelocity(Vector3::Zero), mUseNewtonian(newtonian)
      {
	}
	...
	inline void SetNetForce(Vector3 force)
	{
		if (mUseNewtonian)
		{
			mNetForce = force;
