...
struct Vector2
{
	...
	// Vector addition (a + b)
	inline friend Vector2 operator+(const Vector2 &a, const Vector2 &b) { return Vector2(a.x + b.x, a.y + b.y); }
	...
	inline static Vector2 Lerp(const Vector2 &a, const Vector2 &b, float f) { return Vector2(a + f * (b - a)); }

	// Reflect V about (normalized) N
	inline static Vector2 Reflect(const Vector2 &v, const Vector2 &n) { return v - 2.0f * Vector2::Dot(v, n) * n; }
...
struct Vector3
{
	...
	// Lerp from A to B by f
	inline static Vector3 Lerp(const Vector3 &a, const Vector3 &b, float f) { return Vector3(a + f * (b - a)); }

	// Reflect V about (normalized) N
	inline static Vector3 Reflect(const Vector3 &v, const Vector3 &n) { return v - 2.0f * Vector3::Dot(v, n) * n; }
...
struct Matrix3
{
	...
	inline Matrix3() { *this = Matrix3::Identity; }

	inline explicit Matrix3(float inMat[3][3]) { memcpy(mat, inMat, 9 * sizeof(float)); }
	...
	inline Matrix3 &operator*=(const Matrix3 &right)
	{
		*this = *this * right;
		return *this;
	}
	...
	inline static Matrix3 CreateScale(float scale) { return CreateScale(scale, scale); }
...
struct Matrix4
{
	...
	inline Matrix4() { *this = Matrix4::Identity; }
	...
	inline Vector3 GetXAxis() const { return Vector3::Normalize(Vector3(mat[0][0], mat[0][1], mat[0][2])); }

	// Get the Y axis of the matrix (left)
	inline Vector3 GetYAxis() const { return Vector3::Normalize(Vector3(mat[1][0], mat[1][1], mat[1][2])); }

	// Get the Z axis of the matrix (up)
	inline Vector3 GetZAxis() const { return Vector3::Normalize(Vector3(mat[2][0], mat[2][1], mat[2][2])); }
	...
	inline static Matrix4 CreateScale(const Vector3 &scaleVector)
	{
		return CreateScale(scaleVector.x, scaleVector.y, scaleVector.z);
	}

	// Create a scale matrix with a uniform factor
	inline static Matrix4 CreateScale(float scale) { return CreateScale(scale, scale, scale); }
	...
	inline static Matrix4 CreateLookAt(const Vector3 &eye, const Vector3 &target, const Vector3 &up)
	{
	...
	inline static Matrix4 CreateOrtho(float width, float height, float near, float far)
	{
	...
	inline static Matrix4 CreatePerspectiveFOV(float fovY, float width, float height, float near, float far)
	{
...
struct Quaternion
{
	...
	inline Quaternion() { *this = Quaternion::Identity; }

	// This directly sets the quaternion components --
	// don't use for axis/angle
	inline explicit Quaternion(float inX, float inY, float inZ, float inW) { Set(inX, inY, inZ, inW); }
	...
	inline static Quaternion Slerp(const Quaternion &a, const Quaternion &b, float f)
	{
	...
	inline static Quaternion Concatenate(const Quaternion &q, const Quaternion &p)
	{
		Quaternion retVal;
