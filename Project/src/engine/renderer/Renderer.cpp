...
void Renderer::LinkShaderToMeshComp(const std::string &fileName,
...
void Renderer::LoadShader(Shader *sh) {
  ...
  mProjection = Matrix4::CreatePerspectiveFOV(
      Math::ToRadians(70.f), mScreenWidth, mScreenHeight, 25.0f, 10000.f);
...
void Renderer::SetLightUniforms(Shader *shader) {
  ...
  shader->SetVectorUniform("uDirLight.mDirection",
                           mDirectionalLight.mDirection);
  shader->SetVectorUniform("uDirLight.mDiffuseColor",
                           mDirectionalLight.mDiffuseColor);
  shader->SetVectorUniform("uDirLight.mSpecColor",
                           mDirectionalLight.mSpecColor);
  for (int i = 0; i < mPointLights.size(); ++i) {
    shader->SetVectorUniform("uPointLights[" + std::to_string(i) +
                                 "].mPosition",
    ...
    shader->SetFloatUniform("uPointLights[" + std::to_string(i) + "].mRadius",
                            mPointLights[i].mRadius);
    shader->SetFloatUniform("uPointLights[" + std::to_string(i) +
                                "].mIntensity",
