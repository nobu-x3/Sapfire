...
def write_sfmesh(context, filepath, vertFormat):
    # Get current object
...
class ExportSFMesh(Operator, ExportHelper):
    """Export to SapFire mesh format"""
    bl_idname = "export_test.sfmesh"  # important since its how bpy.ops.import_test.some_data is constructed
    ...
    def execute(self, context):
        return write_sfmesh(context, self.filepath, self.vertFormat)


# Only needed if you want to add into a dynamic menu
def menu_func_export(self, context):
    self.layout.operator(ExportSFMesh.bl_idname, text="GPMesh Exporter (.sfmesh)")


def register():
    bpy.utils.register_class(ExportSFMesh)
    bpy.types.INFO_MT_file_export.append(menu_func_export)


def unregister():
    bpy.utils.unregister_class(ExportSFMesh)
...
if __name__ == "__main__":
    ...
    # test call
    bpy.ops.export_test.sfmesh('INVOKE_DEFAULT')
