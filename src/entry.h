...
b8 create_game (game* out_game) { return instance.create_game (out_game); }

void game_shutdown (game* game) { instance.game_shutdown (game); }

void set_create_game (PFN_create_game fn) { instance.create_game = fn; }

void set_game_shutdown (PFN_game_shutdown fn) { instance.game_shutdown = fn; }
...
int start () {
	...
	if (!game_instance.render || !game_instance.initialize ||
