...
void vulkan_swapchain_create (vulkan_context *context, u32 width, u32 height,
	...
	if (!found) {
		SF_WARNING (
			"VK_FORMAT_B8G8R8A8_UNORM format not found, using "
			"fallback.");
	...
	swapchain_extent.height =
		CLAMP (swapchain_extent.height, min_extent.height, max_extent.height);
	u32 image_count =
    ...
    image_count = image_count > 3 ? 3 : image_count;
	out_swapchain->max_frames_in_flight			   = image_count - 1;
	VkSwapchainCreateInfoKHR swapchain_create_info = {
		VK_STRUCTURE_TYPE_SWAPCHAIN_CREATE_INFO_KHR};
	swapchain_create_info.imageUsage	= VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT;
	swapchain_create_info.minImageCount = image_count;
	swapchain_create_info.imageFormat	= out_swapchain->surface_format.format;
	...
	if (context->device.graphics_queue_index ==
		...
		u32 q_family_indices[] = {(u32)context->device.graphics_queue_index,
	...
	VK_ASSERT_SUCCESS (vkGetSwapchainImagesKHR (context->device.logical_device,
												out_swapchain->handle,
...
void vulkan_swapchain_destroy (vulkan_context *context,
	...
	for (u32 i = 0; i < swapchain->image_count; ++i) {
		vkDestroyImageView (context->device.logical_device,
							swapchain->image_views[i], context->allocator);
	}
