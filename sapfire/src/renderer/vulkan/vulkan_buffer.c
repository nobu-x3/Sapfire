...
b8 vulkan_buffer_create (vulkan_context *context, u64 size,
	...
	if (out_buffer->mem_index == -1) {
		SF_ERROR (
			"Failed to create buffer due to incompatible memory "
			"requirements.");
		return FALSE;
	}
...
b8 vulkan_buffer_resize (vulkan_context *context, u64 new_size,
	...
	VK_ASSERT_SUCCESS (vkAllocateMemory (context->device.logical_device,
					   ...
					   "memory for buffer.");
	VK_ASSERT_SUCCESS (
		vkBindBufferMemory (context->device.logical_device, new_buffer, new_mem,
							0),
		"Failed to resize buffer because failed to bind the new buffer.");
	vulkan_buffer_copy (context, queue, cmd_pool, buffer->handle, 0, new_buffer,
						0, buffer->size);
...
void vulkan_buffer_copy (vulkan_context *context, VkQueue queue,
	...
	vulkan_command_buffer_alloc_and_begin_single_use (context, cmd_pool, &temp);
