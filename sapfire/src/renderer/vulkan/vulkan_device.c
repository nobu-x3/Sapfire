...
b8 vulkan_device_create (vulkan_context *context) {
	...
	b8 transfer_shares_present = context->device.transfer_queue_index ==
								 context->device.present_queue_index;
	if (!present_shares_graphics) { index_count += 1; }
	if (!transfer_shares_graphics) { index_count += 1; }
	u32 indices[32];
	u8 index	   = 0;
	...
	for (u32 i = 0; i < index_count; ++i) {
		q_create_infos[i].sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;
	...
	VK_ASSERT_SUCCESS (vkCreateDevice (context->device.physical_device,
	...
	VK_ASSERT_SUCCESS (
		vkCreateCommandPool (context->device.logical_device, &pool_create_info,
							 context->allocator,
...
b8 select_physical_device (vulkan_context *context) {
	...
	VK_ASSERT_SUCCESS (vkEnumeratePhysicalDevices (
						   context->instance, &phys_device_count, phys_devices),
					   "Failed to enumerate physical devices.");
	...
	for (u32 i = 0; i < phys_device_count; ++i) {
		...
		if (result) {
			SF_INFO ("Selected device: '%s'.", properties.deviceName);
			// GPU type, etc.
			...
			for (u32 j = 0; j < memory.memoryHeapCount; ++j) {
				...
				if (memory.memoryHeaps[j].flags &
					...
					SF_INFO ("Local GPU memory: %.2f GiB", memory_size_gib);
				} else {
					SF_INFO ("Shared System memory: %.2f GiB", memory_size_gib);
				}
			}
	...
	if (!context->device.physical_device) {
		SF_FATAL (
			"No physical devices were found which meet the "
			"requirements.");
		return FALSE;
	}
...
void vulkan_device_query_swapchain_support (
	...
	VK_ASSERT_SUCCESS (
		vkGetPhysicalDeviceSurfaceFormatsKHR (
			physical_device, surface, &out_support_info->format_count, SF_NULL),
		"Failed to query physical device surface formats.");

	if (out_support_info->format_count != 0) {
		if (!out_support_info->formats) {
			out_support_info->formats = sfalloc (
...
b8 physical_device_meets_requirements (
	...
	if (requirements->discrete_gpu) {
		if (properties->deviceType != VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) {
			SF_INFO (
				"Device is not a discrete GPU, and one is "
				"required. Skipping.");
			return FALSE;
		}
	}
	...
	for (u32 i = 0; i < queue_family_count; ++i) {
		...
		VK_ASSERT_SUCCESS (vkGetPhysicalDeviceSurfaceSupportKHR (
							   device, i, surface, &supports_present),
						   "Failed to query physical device surface support.");
		if (supports_present) { out_queue_info->present_family_index = i; }
	}
	...
	if ((!requirements->graphics ||
		...
		if (out_swapchain_support->format_count < 1 ||
			...
			SF_INFO (
				"Required swapchain support not present, "
				"skipping device.");
			return FALSE;
		}
		...
		if (requirements->device_extension_names) {
			...
			if (available_extension_count != 0) {
				available_extensions = sfalloc (sizeof (VkExtensionProperties) *
													available_extension_count,
												MEMORY_TAG_RENDERER);
				VK_ASSERT_SUCCESS (vkEnumerateDeviceExtensionProperties (
				...
				u32 required_extension_count =
					vector_len (requirements->device_extension_names);
				for (u32 i = 0; i < required_extension_count; ++i) {
					b8 found = FALSE;
					for (u32 j = 0; j < available_extension_count; ++j) {
						if (sfstreq (requirements->device_extension_names[i],
		...
		if (requirements->sampler_anisotropy && !features->samplerAnisotropy) {
			SF_INFO (
				"Device does not support samplerAnisotropy, "
				"skipping.");
			return FALSE;
		}
