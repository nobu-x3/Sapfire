...
b8 vulkan_fence_wait (vulkan_context *context, VkFence *fence, u64 timeout_ns) {
	VkResult result = vkWaitForFences (context->device.logical_device, 1, fence,
									   TRUE, timeout_ns);
	switch (result) {
		case VK_SUCCESS: return TRUE;
		case VK_TIMEOUT: SF_ERROR ("vulkan_fence_wait: timed out"); break;
		...
		case VK_ERROR_OUT_OF_HOST_MEMORY:
			...
			break;
		default: SF_ERROR ("vulkan_fence_wait: unknown error."); break;
	}
	return FALSE;
}

void vulkan_fence_reset (vulkan_context *context, VkFence *fence) {
	VK_ASSERT_SUCCESS (vkResetFences (context->device.logical_device, 1, fence),
