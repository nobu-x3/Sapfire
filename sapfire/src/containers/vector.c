...
void* _vector_create(u64 length, u64 stride) {
  ...
  u64* new_vec = sfalloc(header_size + vec_size, MEMORY_TAG_UNKNOWN);
...
void* _vector_resize(void* vector) {
  ...
  void* temp =
      _vector_create((VECTOR_RESIZE_FACTOR * vector_capacity(vector)), stride);
...
void* _vector_pop_at(void* vector, u64 index, void* dest) {
  ...
  if (index != len - 1) {
    sfmemcpy((void*)(addr + (index * stride)),
             (void*)(addr + ((index + 1) * stride)), (len - index) * stride);
  }
  _vector_field_set(vector, VECTOR_LENGTH, len - 1);
  return vector;
}
