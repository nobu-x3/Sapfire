...
b8 platform_init (platform_state *plat_state, const char *app_name, i32 x,
	...
	state->window = SDL_CreateWindow (
		app_name, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height,
	...
	while (SDL_PollEvent (&e) > 0) {
		switch (e.type) {
			...
			case SDL_MOUSEBUTTONDOWN:
				input_process_mouse_button ((mouse_button)e.button.button,
											TRUE);
				break;
			...
			case SDL_WINDOWEVENT:
				switch (e.window.event) {
					case SDL_WINDOWEVENT_RESIZED:
						...
						event_fire (EVENT_CODE_WINDOW_RESIZED, state, context);
						break;
					default: break;
				}
				break;
...
void platform_console_write (const char *message, log_level level) {
	switch (level) {
		...
		case LOG_LEVEL_INFO:
			SDL_LogInfo (SDL_LOG_CATEGORY_APPLICATION, "%s", message);
			break;
		case LOG_LEVEL_TRACE: SDL_Log ("%s", message); break;
